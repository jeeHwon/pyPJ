...
def sell_etf():
    ...
    try:
        ...
        for i in range(cpBalance.GetHeaderValue(7)):
            ...
            if stock_returns > overline or stock_returns < underline:# 수익률 특정 퍼센트 이상 이하일 때 매도 
...
if __name__ == '__main__': 
    try:
        ...
        # 볼린저밴드 해당 종목 추가 
        with open('C:/myData/BuyList/BB_buylist.csv', 'r', encoding='utf-8') as f:
            rdr = csv.reader(f) 
        ...
        target_buy_count = 10 - len(bought_list)    # 매수할 종목 수
        buy_percent = 0.0
        if target_buy_count > 0:
            buy_percent = 0.098 * (10/target_buy_count)
