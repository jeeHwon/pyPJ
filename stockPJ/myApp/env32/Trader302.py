...
# ======================== ver 3.02 ========================
# 201215 복기 : 매수시점 탁월, 장 마감시간 일괄 매도가 문제
# 업데이트 내용 : 수익률 5%이상 되면 매도 기능 추가
# - sell_etf() 메소드 추가
# target : 10
# price percent : 0.095
# target_buy_price = today_open + (lastday_high - lastday_low) * 0.5
# list = [ETF static] + [BB_TF] 
# target_sell_price = 수익률 : 5.0% 돌파
...
def sell_etf():
    """수익률이 일정 퍼센트 이상 종목을 최유리 지정가 IOC 조건으로 매도한다."""
    try:
        cpTradeUtil.TradeInit()
        acc = cpTradeUtil.AccountNumber[0]          # 계좌번호
        accFlag = cpTradeUtil.GoodsList(acc, 1)     # -1:전체, 1:주식, 2:선물/옵션
        ...
        for i in range(cpBalance.GetHeaderValue(7)):
            ...
            if stock_returns > 5.0:                     # 수익률 특정 퍼센트 이상일 때 매도 
