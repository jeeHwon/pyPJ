...
# ======================== ver 3.07 ========================
# 201222 분석 : matplotlib으로 전체 데이터 분석완료 -> 결과 따른 라인 조정 요망

# 업데이트 내용 : 
# 1) myApp/env32/AZ_0TdToRs.py로 금일 체결내역 myData/TodayResult에 res와 pls로 저장
# 2) myApp/env64/AZ_1RsToSt.py로 csv 파일 읽어 가공하여 myData/TodayStatus에 stat으로 저장
# 3) myApp/env64/AZ_2StToDf.py로 익절라인 및 손절라인 따른 수익률 df로 저장
# 4) myApp/env64/AZ_3DfToPl.py로 최적 익절라인 및 손절라인 분석
# 5) 익절라인 조정 : 2.2 -> 1.3%
# 6) 손절라인 유지 : 20.0%
# target : 10
# price percent : 0.098
